...
class NumberZapExtSetupScreen(Screen, ConfigListScreen):
	...
	def initConfig(self):
		...
		self.cfg_fdelay = getConfigListEntry(_("Time to wait first keypress (ms)") + _("/0 - EPG"), self.NZE.first_delay)
		self.cfg_kdelay = getConfigListEntry(_("Time to wait next keypress (ms)") + _("/0 - EPG"), self.NZE.kdelay)
